"""
audit_logger.py — Immutable Audit Trail via Kinesis Firehose (Owner: M6)
CDO-01 · TF3 Self-Heal Engine · sqs-worker
Responsibilities:
  - Ghi log kiểm toán bất biến (immutable audit) cho mọi hành động self-heal.
  - Đẩy record JSON trực tiếp vào AWS Kinesis Data Firehose stream.
  - Firehose tự động chuyển tiếp → S3 Audit Bucket (Object Lock COMPLIANCE ≥90 ngày).
Constraints (project-rules.md §V.3, SUBTEAM2_WORKING_DOC §3.6):
  - Stream name: "tf3-cdo1-sandbox-audit-stream"
  - IRSA Role: "tf3-cdo1-sandbox-irsa-audit-writer"
  - Ghi mọi mốc: INCIDENT_START, DETECT, DECIDE, EXECUTE_START, EXECUTE_DONE,
    VERIFY, ROLLBACK, ESCALATE, CIRCUIT_BREAKER_OPEN.
  - Partition theo tenant_id trong S3 path.
  - Apply scrub() trước khi gửi — SOC2 compliance (không lọt PII/secret).
  - Hỗ trợ FIREHOSE_ENDPOINT_URL env var cho LocalStack testing.
"""
